...
public class GuiLiner {
    public static BiConsumer<VertexConsumer,Matrix4f> line(float x1, float y1, float x2, float y2, float width, Color color){
        ...
        float vertexY1 = y1 - offsetY,
              ...
              vertexY3 = y2 - offsetY,
              vertexY4 = y2 + offsetY;
        float colorR = color.getRedPercentage(),
                colorG = color.getGreenPercentage(),
                colorB = color.getBluePercentage(),
                colorA = color.getAlphaPercentage();
        return (consumer,matrix4f)->{
            consumer.vertex(matrix4f,vertexX1,vertexY1,0).color(colorR,colorG,colorB,colorA).endVertex();
            consumer.vertex(matrix4f,vertexX3,vertexY3,0).color(colorR,colorG,colorB,colorA).endVertex();
            consumer.vertex(matrix4f,vertexX4,vertexY4,0).color(colorR,colorG,colorB,colorA).endVertex();
            consumer.vertex(matrix4f,vertexX2,vertexY2,0).color(colorR,colorG,colorB,colorA).endVertex();
        };
    }
